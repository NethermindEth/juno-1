...
func (q *queue) MaxConfirmed(l1Height uint64) *contract.StarknetLogStateUpdate {
	maxConfirmed := new(contract.StarknetLogStateUpdate)
	for l1BlockNumber, possibleHead := range q.q {
		if l1Height-l1BlockNumber >= q.confirmationPeriod {
			...
			if l1BlockNumber >= maxConfirmed.Raw.BlockNumber {
	...
	// maxConfirmed wasn't set, so there are no confirmed logs.
	if maxConfirmed.BlockNumber == nil {
		return nil
	}
